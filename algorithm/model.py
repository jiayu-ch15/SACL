...
class Policy(nn.Module):
        ...
        if action_space.__class__.__name__ == "Discrete":
            ...
            for num_actions in action_size:
                self.dists.append(Categorical(
                    self.base.output_size, num_actions, gain))
            self.dists = nn.ModuleList(self.dists)
        else:  # discrete+continous
            self.mixed_action = True
            continous = action_space[0].shape[0]
            discrete = action_space[1].n
            self.dist = nn.ModuleList([DiagGaussian(self.base.output_size, continous), Categorical(
                self.base.output_size, discrete, gain)])
    ...
    def act(self, share_inputs, inputs, rnn_hxs_actor, rnn_hxs_critic, masks, available_actions=None, deterministic=False):
        ...
        value, actor_features, rnn_hxs_actor, rnn_hxs_critic = self.base(
            share_inputs, inputs, rnn_hxs_actor, rnn_hxs_critic, masks)

        if self.mixed_action:
            dist, action, action_log_probs = [
            ...
            action_log_probs_out = torch.sum(
                torch.cat(action_log_probs_out, -1), -1, keepdim=True)
    ...
    def get_value(self, share_inputs, inputs, rnn_hxs_actor, rnn_hxs_critic, masks):
        ...
        value, _, rnn_hxs_actor, rnn_hxs_critic = self.base(
            share_inputs, inputs, rnn_hxs_actor, rnn_hxs_critic, masks)
    ...
    def evaluate_actions(self, share_inputs, inputs, rnn_hxs_actor, rnn_hxs_critic, action, masks, active_masks=None):
        ...
        if self.mixed_action:
            ...
            b = b.long()
            action = [a, b]
            dist, action_log_probs, dist_entropy = [
                None, None], [None, None], [None, None]
            for i in range(2):
                ...
                if active_masks is not None:
                    dist_entropy[i] = (
                        dist[i].entropy()*active_masks.squeeze(-1)).sum()/active_masks.sum()
                else:
            ...
            for i in range(self.discrete_N):
                ...
                if active_masks is not None:
                    dist_entropy.append(
                        (dist.entropy()*active_masks.squeeze(-1)).sum()/active_masks.sum())
                else:
...
class NNBase(nn.Module):
    def __init__(self, obs_shape, share_obs_shape, naive_recurrent=False, recurrent=False, hidden_size=64, recurrent_N=1,
                 ...
                 use_common_layer=False, use_orthogonal=True, use_ReLU=False, use_cat_self=True):
        ...
        if self._attn:
            self.encoder_actor = Encoder(obs_shape, attn_size, attn_N, attn_heads,
                                         dropout, use_average_pool, use_orthogonal, use_ReLU, use_cat_self)
            self.encoder_critic = Encoder(share_obs_shape, attn_size, attn_N, attn_heads,
                                          dropout, use_average_pool, use_orthogonal, use_ReLU, use_cat_self)
        ...
        if self._recurrent or self._naive_recurrent:
            ...
            if not self._use_common_layer:
                self.gru_critic = nn.GRU(
                    hidden_size, hidden_size, num_layers=recurrent_N)
    ...
    def _forward_gru_critic(self, x, hxs, masks):
        if x.size(0) == hxs.size(0):
            ...
            for i in range(len(has_zeros) - 1):
                ...
                rnn_scores, hxs = self.gru_critic(
                    x[start_idx:end_idx], hxs * masks[start_idx].view(1, -1, 1))
                outputs.append(rnn_scores)
...
class CNNBase(NNBase):
    def __init__(self, obs_shape, num_agents, naive_recurrent=False, recurrent=False, hidden_size=64, attn=False, attn_size=512, attn_N=2, attn_heads=8, dropout=0.05, use_average_pool=True, use_common_layer=False, use_feature_normalization=False, use_feature_popart=False, use_orthogonal=True, layer_N=1, use_ReLU=False):
        super(CNNBase, self).__init__(obs_shape, num_agents, naive_recurrent, recurrent, hidden_size,
                                      attn, attn_size, attn_N, attn_heads, dropout, use_average_pool, use_common_layer, use_orthogonal)
        ...
        self.actor = nn.Sequential(
            ...
            Flatten(),
            init_(nn.Linear(32 * (num_image-3+1) * \
                            (num_image-3+1), hidden_size)), nn.ReLU(),
    ...
    def forward(self, agent_id, share_inputs, inputs, rnn_hxs_actor, rnn_hxs_critic, rnn_c_actor, rnn_c_critic, masks):
        ...
        if self._use_common_layer:
            ...
            if self.is_recurrent or self.is_naive_recurrent:
                hidden_actor, rnn_hxs_actor = self._forward_gru(
                    hidden_actor, rnn_hxs_actor, masks)
                hidden_critic, rnn_hxs_critic = self._forward_gru(
                    hidden_critic, rnn_hxs_critic, masks)
            ...
            if self.is_recurrent or self.is_naive_recurrent:
                hidden_actor, rnn_hxs_actor = self._forward_gru(
                    hidden_actor, rnn_hxs_actor, masks)
                hidden_critic, rnn_hxs_critic = self._forward_gru_critic(
                    hidden_critic, rnn_hxs_critic, masks)
...
class MLPBase(NNBase):
    def __init__(self, obs_shape, share_obs_shape, naive_recurrent=False, recurrent=False, hidden_size=64, recurrent_N=1,
        ...
        if self._attn:
            if use_average_pool == True:
                inputs_actor_dim = attn_size + obs_shape[-1][1]
                inputs_critic_dim = attn_size + share_obs_shape[-1][1]
            else:
                inputs_dim = 0
                split_shape = obs_shape[1:]
                for i in range(len(split_shape)):
                    inputs_dim += split_shape[i][0]

                inputs_critic_dim = 0
                split_shape_critic = share_obs_shape[1:]
                for i in range(len(split_shape_critic)):
                    inputs_critic_dim += split_shape_critic[i][0]

                inputs_actor_dim = inputs_dim * attn_size
                inputs_critic_dim = inputs_critic_dim * attn_size

            self.actor_attn_norm = nn.LayerNorm(inputs_actor_dim)
            self.critic_attn_norm = nn.LayerNorm(inputs_critic_dim)
        else:
            inputs_actor_dim = obs_dim
            inputs_critic_dim = share_obs_dim
        ...
        self.actor = MLPLayer(inputs_actor_dim, hidden_size,
                              self._layer_N, self._use_orthogonal, self._use_ReLU)
        self.critic = MLPLayer(inputs_critic_dim, hidden_size,
                               self._layer_N, self._use_orthogonal, self._use_ReLU)

        if self._use_common_layer:
            self.actor = nn.Sequential(
                init_(nn.Linear(inputs_actor_dim, hidden_size)), active_func, nn.LayerNorm(hidden_size))
            self.critic = nn.Sequential(
                init_(nn.Linear(inputs_critic_dim, hidden_size)), active_func, nn.LayerNorm(hidden_size))
    ...
    def forward(self, share_inputs, inputs, rnn_hxs_actor, rnn_hxs_critic, masks):
        ...
        if self.is_attn:
            x = self.encoder_actor(x, self_idx=-1)
            share_x = self.encoder_critic(share_x, self_idx=-1)
    ...
    def __init__(self, split_shape, d_model=512, attn_N=2, heads=8, dropout=0.0, use_average_pool=True, use_orthogonal=True, use_ReLU=False, cat_self=False):
        ...
        if self._cat_self:
            self.embedding = CatSelfEmbedding(
                split_shape[1:], d_model, use_orthogonal, use_ReLU)
        else:
            self.embedding = Embedding(
                split_shape[1:], d_model, use_orthogonal, use_ReLU)
        self.layers = get_clones(EncoderLayer(
            d_model, heads, dropout, use_orthogonal, use_ReLU), self._attn_N)
