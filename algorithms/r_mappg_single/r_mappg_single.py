...
class R_MAPPG():
    ...
    def update_action_log_probs(self, buffer):
        for step in range(buffer.episode_length):
            ...
            action_log_probs = np.array(
                np.split(action_log_probs.detach().cpu().numpy(), buffer.n_rollout_threads))
    ...
    def auxiliary_loss_update(self, sample):
        ...
        # Reshape to do in a single forward pass for all steps
        values, action_log_probs = self.policy.get_value_and_logprobs(
            share_obs_batch, obs_batch, recurrent_hidden_states_batch, recurrent_hidden_states_critic_batch, masks_batch, available_actions_batch)
    ...
    def separated_update(self, agent_id, buffer):
        ...
        for _ in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                action_loss, dist_entropy, actor_grad_norm, kl_loss, ratio = self.policy_loss_update(
                    sample)
