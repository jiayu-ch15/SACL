    ...
    def __init__(self, episode_length, n_rollout_threads, agent_obs_space, agent_share_obs_space, agent_action_space,
        ...
        if len(agent_obs_shape) == 3:
            self.share_obs = np.zeros(
                (episode_length + 1, n_rollout_threads, *agent_share_obs_shape)).astype(np.float32)
            self.obs = np.zeros(
                (episode_length + 1, n_rollout_threads, *agent_obs_shape)).astype(np.float32)
        else:
            self.share_obs = np.zeros(
                (episode_length + 1, n_rollout_threads, agent_share_obs_shape[0])).astype(np.float32)
            self.obs = np.zeros(
                (episode_length + 1, n_rollout_threads, agent_obs_shape[0])).astype(np.float32)
        ...
        self.rewards = np.zeros(
            (episode_length, n_rollout_threads, 1)).astype(np.float32)
        self.value_preds = np.zeros(
            (episode_length + 1, n_rollout_threads, 1)).astype(np.float32)
        ...
        if agent_action_space.__class__.__name__ == 'Discrete':
            self.available_actions = np.ones(
                (episode_length + 1, n_rollout_threads, agent_action_space.n)).astype(np.float32)
            action_shape = 1
        ...
        self.actions = np.zeros(
            (episode_length, n_rollout_threads, action_shape)).astype(np.float32)
        self.masks = np.ones(
            (episode_length + 1, n_rollout_threads, 1)).astype(np.float32)
