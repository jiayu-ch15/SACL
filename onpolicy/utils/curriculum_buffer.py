...
class CurriculumBuffer(object):
    ...
    def __init__(self, buffer_size, update_method="fps", sample_method="uniform", max_staleness=5):
        self.eps = 1e-10
        self.buffer_size = buffer_size
        self.update_method = update_method
        self.sample_method = sample_method
        self.max_staleness = max_staleness
        # self.max_staleness = np.inf

        self._state_buffer = np.zeros((0, 1), dtype=np.float32)
        self._weight_buffer = np.zeros((0, 1), dtype=np.float32)
        self._staleness_buffer = np.zeros((0, 1), dtype=np.int32)
    ...
    def insert(self, states, weights=None):
        ...
        if weights is None:
            self._temp_weight_buffer.extend([0 for _ in range(len(states))])
        else:
    ...
    def update(self):
        ...
        all_staleness = np.zeros(len(all_weights), dtype=np.int32)

        # remove states and weight with staleness >= max_staleness
        self._staleness_buffer += 1
        remain_idx = (self._staleness_buffer < self.max_staleness)
        self._state_buffer = self._state_buffer[remain_idx]
        self._weight_buffer = self._weight_buffer[remain_idx]
        self._staleness_buffer = self._staleness_buffer[remain_idx]
        ...
        if self._state_buffer.shape[0] != 0:  # state buffer is not empty
            ...
            all_staleness = np.concatenate([self._staleness_buffer, all_staleness], axis=0)
        ...
        if all_states.shape[0] <= self.buffer_size:
            ...
            self._staleness_buffer = copy.deepcopy(all_staleness)
        else:
            if self.update_method == "random":
                ...
                self._staleness_buffer = copy.deepcopy(all_staleness[random_idx])
                ...
                self._staleness_buffer = copy.deepcopy(all_staleness[fps_idx])
            else:
    ...
    def sample(self, num_samples):
        ...
        elif self.sample_method == "prioritized":
