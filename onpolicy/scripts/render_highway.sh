...
do
<<<<<<< HEAD
    CUDA_VISIBLE_DEVICES=4 python render/render_highway.py --env_name ${env} --algorithm_name ${algo} --experiment_name ${exp} --scenario_name ${scenario} --task_type ${task} --n_attackers ${n_attackers} --n_defenders ${n_defenders} --n_dummies ${n_dummies} --seed ${seed} --n_training_threads 1 --n_render_rollout_threads 1 --horizon 40 --use_render --save_gifs --use_wandb --model_dir "results/Highway/highway-v0/rmappo/debug/run12/models"
=======
...
>>>>>>> b2f7b7aed6e2adb356d0c00bf350786933836618
done
