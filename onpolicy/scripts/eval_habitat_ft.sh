...
do
    echo "seed is ${seed}:"
    CUDA_VISIBLE_DEVICES=0 python eval/eval_habitat_ft.py --scenario_name ${scenario} \
    --env_name ${env} --algorithm_name ${algo} --experiment_name ${exp} --num_agents ${num_agents} \
    --split "train" --use_same_scene --scene_id 16 --eval_episodes 100 --use_eval --ifi 0.01 --seed 1 \
