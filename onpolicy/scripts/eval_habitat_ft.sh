...
exp="seed2_dis2_1agents_middle_61"
seed_max=1
...
do
    echo "seed is ${seed}:"
    CUDA_VISIBLE_DEVICES=7 python eval/eval_habitat_ft.py --scenario_name ${scenario} \
    --env_name ${env} --algorithm_name ${algo} --experiment_name ${exp} --num_agents ${num_agents} \
    --split "train" --use_same_scene --scene_id 61 --eval_episodes 100 --use_eval --ifi 0.01 --seed 2 \
    ...
    --use_max --use_max_map --local_planner fmm --use_merge_local --use_different_start_pos --use_wandb #--use_fixed_start_pos
