...
do
    ...
    CUDA_VISIBLE_DEVICES=0 python train/train_highway.py --env_name ${env} --algorithm_name ${algo} --experiment_name ${exp} --scenario_name ${scenario} --task_type ${task} --n_attackers ${n_attackers} --n_defenders ${n_defenders} --n_dummies ${n_dummies} --seed ${seed} --n_training_threads 1 --n_rollout_threads 1 --n_eval_rollout_threads 1 --horizon 40 --episode_length 40 --log_interval 1 --use_wandb --use_render --cuda --policy_path ../envs/highway/agents/policy_pool/DQN/model/dueling_ddqn_obs25_act5_baseline.tar
done
