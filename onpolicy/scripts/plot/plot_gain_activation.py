...
map_names = ['MMM2', '3s5z_vs_3s6z', '3s_vs_5z','25m','8m_vs_9m']

for map_name in map_names:
    plt.figure()
    ###################################PPO###################################
    exp_names = ['mappo_relu_gain0.01', 'mappo_relu_gain1', 'mappo_tanh_gain0.01', 'mappo_tanh_gain1'] 
    label_names = ["ReLU,0.01", "ReLU,1", "Tanh,0.01", "Tanh,1"]
    color_names = ['red','blue','limegreen','saddlebrown']

    save_dir = './gain_and_activation/'
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    max_steps = []
    for exp_name, label_name, color_name in zip(exp_names, label_names, color_names):
        print(exp_name)
        data_dir =  './gain_and_activation/' + map_name + '/' + map_name + '_' + exp_name + '.csv'
    ...
    plt.savefig(save_dir + map_name + "_gain_and_activation.png", bbox_inches="tight")