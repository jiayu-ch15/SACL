...
map_names = ['3s5z_vs_3s6z','MMM2','10m_vs_11m','8m_vs_9m','2s3z']
title_names = [name.replace("_vs_"," vs. ") for name in map_names]
exp_names = ['mappo_mini1', 'mappo_mini2', 'mappo_mini4'] 
...
for map_name, title_name, ax in zip(map_names, title_names, axes):
    ...
    for exp_name, label_name, color_name in zip(exp_names, label_names, color_names):
        print(exp_name)
        data_dir =  './mini_batch_size/' + map_name + '/' + map_name + '_' + exp_name + '.csv'
...
plt.savefig(save_dir + "mini_batchsize_subplot_rightlegend.png", bbox_inches="tight")