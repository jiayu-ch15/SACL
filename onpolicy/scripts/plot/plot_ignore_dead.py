...
map_names = ['MMM2','6h_vs_8z','corridor','10m_vs_11m','3s5z_vs_3s6z','27m_vs_30m','5m_vs_6m','3s_vs_4z','3s_vs_5z']
...
for map_name, title_name in zip(map_names,title_names):
    plt.figure()
    ###################################PPO###################################
    exp_names = ['final_mappo', 'final_ignore']
    label_names = ["zero state", "ignore state"]
    color_names = ['red','blue']

    save_dir = './ignore_dead/'
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    max_steps = []
    for exp_name, label_name, color_name in zip(exp_names, label_names, color_names):
        print(exp_name)
        if exp_name in ['final_ignore']:
            data_dir =  './ignore_dead/' + map_name + '/' + map_name + '_' + exp_name + '.csv'
        else:
    ...
    plt.savefig(save_dir + map_name + "_ignore_dead.png", bbox_inches="tight")