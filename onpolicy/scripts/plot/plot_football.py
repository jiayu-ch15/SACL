...
label_names = ['MAPPO','QMix','CDS(QMix-d)','CDS(QPlex-d)','CDS(QMix)','CDS(QPlex)','TiKick']
...
for scenario_name, title_name in zip(scenario_names, title_names):
    ...
    x_major_locator = MultipleLocator(int(max_step/10))
    x_minor_Locator = MultipleLocator(int(max_step/5)) 
