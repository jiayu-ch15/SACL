...
for map_name, title_name in zip(map_names,title_names):
    plt.figure()
    ###################################PPO###################################
    exp_names = ['final_mappo', 'mappo_noagentid']
    label_names = ["with agent id", "without agent id"]
    ...
    for exp_name, label_name, color_name in zip(exp_names, label_names, color_names):
        print(exp_name)
        if exp_name in ['mappo_noagentid']:
    ...
    plt.savefig(save_dir + map_name + "_agentid.png", bbox_inches="tight")