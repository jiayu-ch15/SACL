...
class R_TRPO():
    ...
    def ppo_update(self, sample, turn_on=True):
        ...
        value_preds_batch, return_batch, masks_batch, active_masks_batch, old_probs_batch, old_action_log_probs_batch, \
        ...
        old_log_probs_batch = torch.log(old_probs_batch + eps.float().detach())
        eps = (probs==0) * 1e-8
        log_probs = torch.log(probs + eps.float().detach())

        if self._use_forward_kl:
        ...
        if turn_on:
            (policy_loss + kl_loss * self.kl_coef - dist_entropy * self.entropy_coef).backward()
