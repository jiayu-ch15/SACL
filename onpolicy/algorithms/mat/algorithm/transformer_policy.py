...
class TransformerPolicy_ensemble:
    """
    ...
    def get_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, masks, available_actions=None,
        ...
        values = values.view(-1, self.num_critic)
    ...
    def get_values(self, cent_obs, rnn_states_critic, masks):
        ...
        for critic_id in range(self.num_critic):
            values = self.transformer.get_values_seperate(cent_obs, critic_id)
            values_all.append(values)
        values_all = torch.stack(values_all, dim=1)
        # values = self.transformer.get_values(cent_obs)

        values_all = values_all.view(-1, self.num_critic)

        return values_all

    def get_values_seperate(self, cent_obs, rnn_states_critic, masks, critic_id):
        """
        Get value function predictions.
        :param cent_obs (np.ndarray): centralized input to the critic.
        :param rnn_states_critic: (np.ndarray) if critic is RNN, RNN states for critic.
        ...
        # values = self.transformer.get_values(cent_obs, obs)
        # values = self.transformer.get_values(cent_obs, critic_id)
        values = self.transformer.get_values_seperate(cent_obs, critic_id)
    ...
    def only_evaluate_actions(self, obs, rnn_states_actor, actions, masks,
        ...
        action_log_probs, values, entropy = self.transformer.only_evaluate_actions(obs, actions, available_actions)
        ...
        return action_log_probs, entropy, policy_values
