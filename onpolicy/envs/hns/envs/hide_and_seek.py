...
from onpolicy.envs.hns.wrappers.multi_agent import (SplitMultiAgentActions, JoinMultiAgentActions,
                                           SplitObservations, SelectKeysWrapper)
from onpolicy.envs.hns.wrappers.util import (DiscretizeActionWrapper, ConcatenateObsWrapper,
                                    MaskActionWrapper, SpoofEntityWrapper,
                                    DiscardMujocoExceptionEpisodes,
                                    AddConstantObservationsWrapper)
from onpolicy.envs.hns.wrappers.manipulation import (GrabObjWrapper, GrabClosestWrapper,
...
from onpolicy.envs.hns.wrappers.line_of_sight import (AgentAgentObsMask2D, AgentGeomObsMask2D,
                                             AgentSiteObsMask2D)
from onpolicy.envs.hns.wrappers.prep_phase import (PreparationPhase, NoActionsInPrepPhase,
                                          MaskPrepPhaseAction)
...
class TrackStatWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if self.in_prep_phase and obs['prep_obs'][0, 0] == 1.0:
            ...
            if self.n_boxes > 0:
                self.max_box_move_prep = np.max(np.linalg.norm(
                    obs['box_pos'] - self.box_pos_start, axis=-1))
                self.num_box_lock_prep = np.sum(obs['obj_lock'])
            if self.n_ramps > 0:
                self.max_ramp_move_prep = np.max(np.linalg.norm(
                    obs['ramp_pos'] - self.ramp_pos_start, axis=-1))
        ...
        if done:
            ...
            if self.n_boxes > 0:
                self.max_box_move = np.max(np.linalg.norm(
                    obs['box_pos'] - self.box_pos_start, axis=-1))
            ...
            if self.n_ramps > 0:
                self.max_ramp_move = np.max(np.linalg.norm(
                    obs['ramp_pos'] - self.ramp_pos_start, axis=-1))
                info.update({
...
class HideAndSeekRewardWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        this_rew[:self.n_hiders][np.any(
            obs['mask_aa_obs'][self.n_hiders:, :self.n_hiders], 0)] = -1.0
        this_rew[self.n_hiders:][~np.any(
            obs['mask_aa_obs'][self.n_hiders:, :self.n_hiders], 1)] = -1.0
...
def HideAndSeekEnv(args, n_substeps=15, horizon=60, deterministic_mode=True,
    ...
    if scenario == 'randomwalls':
        ...
        if hiders_together_radius is not None:
            htr_in_cells = np.ceil(
                hiders_together_radius / cell_size).astype(int)
        ...
        if seekers_together_radius is not None:
            str_in_cells = np.ceil(
                seekers_together_radius / cell_size).astype(int)
    ...
    env.add_module(Agents(n_hiders + n_seekers,
                          ...
                          color=[np.array((66., 235., 244., 255.)) / 255] *
                          n_hiders + [(1., 0., 0., 1.)] * n_seekers,
    ...
    if n_lidar_per_agent > 0 and visualize_lidar:
        env.add_module(LidarSites(n_agents=n_hiders + n_seekers,
    ...
    if n_food > 0:
        ...
        if food_together_radius is not None:
            cell_size = env.floor_size / grid_size
            ftr_in_cells = np.ceil(
                food_together_radius / cell_size).astype(int)

            env.metadata['food_together_radius'] = ftr_in_cells

            assert n_food % n_food_cluster == 0
            cluster_assignments = np.repeat(
                np.arange(0, n_food, n_food // n_food_cluster), n_food // n_food_cluster)
            ...
            food_placement[::n_food //
                           n_food_cluster] = [first_food_placement] * n_food_cluster
        else:
    ...
    keys_self = ['agent_qpos_qvel', 'hider',
                 'prep_obs', 'current_step', 'vector_door_obs']
    ...
    if team_size_obs:
        keys_self += ['team_size']
    env = TeamMembership(env, np.append(
        np.zeros((n_hiders,)), np.ones((n_seekers,))))
    ...
    if n_food:
        env = AgentSiteObsMask2D(
            env, pos_obs_key='food_pos', mask_obs_key='mask_af_obs')
        env = FoodHealthWrapper(env, respawn_time=(np.inf if food_respawn_time is None else food_respawn_time),
                                eat_thresh=(
                                    np.inf if food_radius is None else food_radius),
    ...
    if lock_box and np.max(n_boxes) > 0:
        env = LockObjWrapper(env, body_names=[f'moveable_box{i}' for i in range(np.max(n_boxes))],
                             agent_idx_allowed_to_lock=np.arange(
                                 n_hiders+n_seekers),
    ...
    if n_boxes:
        env = SpoofEntityWrapper(env, np.max(n_boxes), [
    ...
    if max_n_agents is not None:
        env = SpoofEntityWrapper(
            env, max_n_agents, ['agent_qpos_qvel', 'hider', 'prep_obs'], ['mask_aa_obs'])
