...
class HabitatRunner(Runner):
    ...
    def run(self):
        ...
        self.auc_area = np.zeros((self.n_rollout_threads, self.max_episode_length), dtype=np.float32)
        self.auc_single_area = np.zeros((self.n_rollout_threads, self.num_agents, self.max_episode_length), dtype=np.float32)
            ...
            for step in range(self.max_episode_length):
                ...
                for e in range (self.n_rollout_threads):
                    for key in self.sum_env_info_keys:
                        if key in infos[e].keys():
                            ...
                            if key == 'merge_explored_ratio':
                                self.auc_area[e, step] = self.auc_area[e, step-1] + np.array(infos[e][key])
                            if key == 'explored_ratio':
                                self.auc_single_area[e, :, step] = self.auc_single_area[e, :, step-1] + np.array(infos[e][key])
                    ...
                    if self.num_agents==1:
                        if step == 49:
                            self.env_info['50step_merge_auc'][e] = self.auc_area[e,:step+1].sum().copy()
                            self.env_info['50step_auc'][e] = self.auc_single_area[e, :, :step+1].sum(axis =1)    
                        if step == 99:
                            self.env_info['100step_merge_auc'][e] = self.auc_area[e,:step+1].sum().copy()
                            self.env_info['100step_auc'][e] = self.auc_single_area[e, :, :step+1].sum(axis =1)    
                        if step == 149:
                            self.env_info['150step_merge_auc'][e] = self.auc_area[e,:step+1].sum().copy()
                            self.env_info['150step_auc'][e] = self.auc_single_area[e, :, :step+1].sum(axis =1)     
                        if step == 199:
                            self.env_info['200step_merge_auc'][e] = self.auc_area[e,:step+1].sum().copy()
                            self.env_info['200step_auc'][e] = self.auc_single_area[e, :, :step+1].sum(axis =1)    
                        if step == 249:
                            self.env_info['250step_merge_auc'][e] = self.auc_area[e,:step+1].sum().copy()
                            self.env_info['250step_auc'][e] = self.auc_single_area[e, :, :step+1].sum(axis =1) 
                        if step == 299:
                            self.env_info['300step_merge_auc'][e] = self.auc_area[e,:step+1].sum().copy()
                            self.env_info['300step_auc'][e] = self.auc_single_area[e, :, :step+1].sum(axis =1)
                        if step == 349:
                            self.env_info['350step_merge_auc'][e] = self.auc_area[e,:step+1].sum().copy()
                            self.env_info['350step_auc'][e] = self.auc_single_area[e, :, :step+1].sum(axis =1)
                        if step == 399:
                            self.env_info['400step_merge_auc'][e] = self.auc_area[e,:step+1].sum().copy()
                            self.env_info['400step_auc'][e] = self.auc_single_area[e, :, :step+1].sum(axis =1)
                        if step == 449:
                            self.env_info['450step_merge_auc'][e] = self.auc_area[e,:step+1].sum().copy()
                            self.env_info['450step_auc'][e] = self.auc_single_area[e, :, :step+1].sum(axis =1)      
                    else:            
                        if step == 49:
                            self.env_info['50step_merge_auc'][e] = self.auc_area[e,:step+1].sum().copy()
                            self.env_info['50step_auc'][e] = self.auc_single_area[e, :, :step+1].sum(axis =1)    
                        if step == 99:
                            ...
                            self.env_info['100step_auc'][e] =self.auc_single_area[e, :, :step+1].sum(axis =1)    
                        if step == 150:
                            self.env_info['120step_merge_auc'][e] = self.auc_area[e,:step+1].sum().copy()
                            self.env_info['120step_auc'][e] = self.auc_single_area[e, :, :step+1].sum(axis =1)    
                        if step == 149:
                            self.env_info['150step_merge_auc'][e] = self.auc_area[e,:step+1].sum().copy()
                            self.env_info['150step_auc'][e] = self.auc_single_area[e, :, :step+1].sum(axis =1)    
                        if step == 179:
                            self.env_info['180step_merge_auc'][e] = self.auc_area[e,:step+1].sum().copy()
                            self.env_info['180step_auc'][e] = self.auc_single_area[e, :, :step+1].sum(axis =1)    
                        if step == 199:
                            self.env_info['200step_merge_auc'][e] = self.auc_area[e,:step+1].sum().copy()
                            self.env_info['200step_auc'][e] = self.auc_single_area[e, :, :step+1].sum(axis =1)    
                        if step == 249:
    ...
    def init_each_map_and_pose(self, e):
        ...
        self.intrinsic_gt[e] = np.zeros((self.num_agents, self.full_w, self.full_h), dtype=np.float32)
    ...
    def init_each_global_policy(self, e):       
        ...
        if self.use_merge:
            if self.use_seperated_cnn_model:
                if self.use_original_size:
                    self.global_input['global_merge_obs'][e] = np.zeros((self.num_agents, 4, self.full_w, self.full_h), dtype=np.float32)
                else:
                    self.global_input['global_merge_obs'][e] = np.zeros(( self.num_agents, 4, space_w, space_h), dtype=np.float32)
                self.global_input['local_merge_obs'][e] = np.zeros((self.num_agents, 4, space_w, space_h), dtype=np.float32)
            else:
                self.global_input['global_merge_obs'][e] = np.zeros((self.num_agents, 8, space_w, space_h), dtype=np.float32)
                
        if self.use_merge_goal:
            if self.use_original_size:
                self.global_input['global_merge_goal'][e] = np.zeros((self.num_agents, 2, self.full_w, self.full_h), dtype=np.float32)
            else:
                self.global_input['global_merge_goal'][e] = np.zeros((self.num_agents, 2, space_w, space_h), dtype=np.float32)
        
        if self.use_single:
            if self.use_seperated_cnn_model:
                if self.use_original_size:
                    self.global_input['global_obs'][e] = np.zeros(( self.num_agents, 4, self.full_w, self.full_h), dtype=np.float32)
                else:
                    self.global_input['global_obs'][e] = np.zeros(( self.num_agents, 4, space_w, space_h), dtype=np.float32)
                self.global_input['local_obs'][e] = np.zeros(( self.num_agents, 4, space_w, space_h), dtype=np.float32)
            else:
                self.global_input['global_obs'][e] = np.zeros((self.num_agents, 8, space_w, space_h), dtype=np.float32)

        if self.use_orientation:
            self.global_input['global_orientation'][e] = np.zeros((self.num_agents, 1), dtype=np.long)
            self.global_input['other_global_orientation'][e] = np.zeros(( self.num_agents, self.num_agents-1), dtype=np.long)
        
        if self.use_vector_agent_id:    
            self.global_input['vector'][e] = np.zeros(( self.num_agents, self.num_agents), dtype=np.float32)
        ...
        if self.use_cnn_agent_id:
            if self.use_one:
                ...
                vector_cnn_channel = self.num_agents + 1
            if self.use_original_size:
                self.global_input['vector_cnn'][e] = np.zeros(( self.num_agents, vector_cnn_channel, self.full_w, self.full_h), dtype=np.float32)
            else:
                self.global_input['vector_cnn'][e] = np.zeros(( self.num_agents, vector_cnn_channel, space_w, space_h), dtype=np.float32)
        ...
        if self.use_single_agent_trace:
            if self.use_own:
                ...
                vector_cnn_channel = self.num_agents
            if self.use_original_size:
                self.global_input['trace_image'][e] = np.zeros(( self.num_agents, self.num_agents, self.full_w, self.full_h), dtype=np.float32)
            else:
                self.global_input['trace_image'][e] = np.zeros(( self.num_agents, self.num_agents, space_w, space_h), dtype=np.float32)
        
        self.share_global_input = self.global_input.copy()
        
        if self.use_centralized_V:
            if self.use_original_size:
                self.share_global_input['gt_map'][e] = np.zeros(( self.num_agents, 1, self.full_w, self.full_h), dtype=np.float32)
            else:
                self.share_global_input['gt_map'][e] = np.zeros((self.num_agents, 1, space_w, space_h), dtype=np.float32)
        
        self.global_masks[e] = np.ones((self.num_agents, 1), dtype=np.float32) 
        self.global_goal[e] = np.zeros((self.num_agents, 2), dtype=np.float32) 
    ...
    def reset_env_info(self, dones, infos):
        for e, done, info in zip(range(dones.shape[0]), dones, infos):
            if np.all(done):
                ...
                self.merge_explored_gt[e] = info['merge_explored_gt'] 
                self.merge_obstacle_gt[e] = info['merge_obstacle_gt'] 
                self.explored_map[e] = info['explored_map'] 
                self.obstacle_map[e] = info['obstacle_map'] 
                self.init_pose[e] = info['init_pose']
                self.auc_area[e] = np.zeros((self.max_episode_length), dtype=np.float32)
                self.auc_single_area[e] = np.zeros((self.num_agents, self.max_episode_length), dtype=np.float32)
                if not self.use_delta_reward:
                    self.rewards[e] = np.zeros((self.num_agents, 1), dtype=np.float32)   
        ...
        for k, v in auc_infos.items():
            if "merge" not in k:
                for agent_id in range(self.num_agents):
                    ...
                    for i in range(self.max_episode_length):
                        if self.use_wandb:
                            wandb.log({agent_k: np.mean(np.array(v)[:,:,agent_id])}, step=i+1)
                        else:
            ...
            for step in range(self.max_episode_length):
                ...
                for e in range(self.n_rollout_threads):
                    ...
                    if self.num_agents==1:
                        ...
                        if step == 299:
                            self.env_info['300step_merge_auc'][e] = auc_area[e,:step+1].sum().copy()
                            self.env_info['300step_auc'][e] = auc_single_area[e, :, :step+1].sum(axis =1)
                        if step == 349:
                            self.env_info['350step_merge_auc'][e] = auc_area[e,:step+1].sum().copy()
                            self.env_info['350step_auc'][e] = auc_single_area[e, :, :step+1].sum(axis =1)
                        if step == 399:
                            self.env_info['400step_merge_auc'][e] = auc_area[e,:step+1].sum().copy()
                            self.env_info['400step_auc'][e] = auc_single_area[e, :, :step+1].sum(axis =1)
                        if step == 449:
                            self.env_info['450step_merge_auc'][e] = auc_area[e,:step+1].sum().copy()
                            self.env_info['450step_auc'][e] = auc_single_area[e, :, :step+1].sum(axis =1)      
                    else:            
                        if step == 49:
                            self.env_info['50step_merge_auc'][e] = auc_area[e,:step+1].sum().copy()
                            self.env_info['50step_auc'][e] = auc_single_area[e, :, :step+1].sum(axis =1)    
