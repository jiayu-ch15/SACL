    ...
    def __init__(self, config):
        ...
        if "mappo" in self.algorithm_name:
            if self.use_single_network:
                ...
                from onpolicy.algorithms.r_mappo.algorithm.rMAPPOPolicy import R_MAPPOPolicy_ensemble as Policy_ensemble
        ...
        if "ft" not in self.algorithm_name:
            # policy network
            self.red_policy = Policy_ensemble(
                self.all_args,
            ...
            self.blue_policy = Policy_ensemble(
                self.all_args,
            ...
            # buffer
            self.red_buffer = SharedReplayBuffer_ensemble(
                self.all_args,
                self.num_red,
                self.envs.observation_space[0],
                self.envs.share_observation_space[0] if self.use_centralized_V else self.envs.observation_space[0],
            ...
            self.blue_buffer = SharedReplayBuffer_ensemble(
                self.all_args,
                self.num_blue,
                self.envs.observation_space[-1],
                self.envs.share_observation_space[-1] if self.use_centralized_V else self.envs.observation_space[-1],
