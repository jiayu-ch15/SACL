...
class MPERunner(Runner):
    ...
    def eval(self, total_num_steps):
        ...
        print(
            f"adv step reward: {np.sum(eval_adv_step_reward):.2f}, "
            f"good step reward: {np.sum(eval_good_step_reward):.2f}, "
            f"outside per step: {np.sum(eval_outside_per_step):.4f}, "
            f"collision per step: {np.sum(eval_collision_per_step):.4f}.\n"
        )
        # print(
