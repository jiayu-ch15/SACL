...
do
    echo "seed is ${seed}:"
    CUDA_VISIBLE_DEVICES=7 python train/train_hns.py --env_name ${env} --algorithm_name ${algo} --scenario_name ${scenario_name} --num_agents ${num_agents} --num_boxes ${num_boxes} --seed ${seed} --n_training_threads 1 --n_rollout_threads 2 --num_mini_batch 1 --episode_length 150 --num_env_steps 100000000 --ppo_epoch 15 --gain 1 --attn --hidden_size 128 --attn_size 128
